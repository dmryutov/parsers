...
for ($m = 1; $m <= 12; $m++) {
    ...
    mysql_query("SELECT
        ...
        FROM banks_".$y."_".$m2." b LEFT JOIN indicator2 i on b.indicator=i.id;");
    mysql_query("SET @sql = CONCAT('SELECT bl.name \"Название\", bank_id \"Лицензия\", ".
                "\'$m1 $y\' AS \"Дата\", ', @sql, ' FROM banks_".$y."_".$m2." bb LEFT JOIN bank bl ".
                "ON bb.bank_id=bl.number GROUP BY bl.name');") or die(mysql_error());
    ...
    while ($res = mysql_fetch_array($query)) {
        if ($i == 1) {
            foreach ($res as $key => $value) {
                if (!is_int($key)) {
                    $aSheet->setCellValue($lettr[$j].'1', $key);
                    $j++;
                }
            }
            $i++;
        ...
        foreach ($res as $key => $value) {
            if (!is_int($key)) {
                $aSheet->setCellValue($lettr[$j].$i, $value);
                $j++;
            }
        }
        $i++;
